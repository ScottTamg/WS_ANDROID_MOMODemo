...
import com.wushuangtech.library.screenrecorder.gles.EglCore;
import com.wushuangtech.library.screenrecorder.gles.Texture2dProgram;
import com.wushuangtech.library.screenrecorder.gles.WindowSurface;
...
public class TextureMovieEncoder implements Runnable, SurfaceTexture.OnFrameAvailableListener {
     ...
     */
    void startRecording(EncoderConfig config) {
     ...
     */
    private void frameAvailable(SurfaceTexture st) {
     ...
     */
    private void frameAvailable(SurfaceTexture st, long timestamp) {
        synchronized (mReadyFence) {
            if (!mReady) {
                return;
            }
    ...
    private void handleFrameAvailable(float[] transform, long timestampNanos) {
        if (VERBOSE) {
            Log.d(TAG, "handleFrameAvailable tr=" + Arrays.toString(transform));
        }
...
//         saveFirstFrame();
     ...
     */
    private File getCoverFile(File mp4) {
    ...
    /**
     * Author: wangzg <br/>
     * Time: 2017-12-14 10:26:53<br/>
     * Description: 拿到UI传入的EncoderConfig对象，准备开始编码.
     *
...
//        int mVideoHeight = (int) (config.mHeight * (1f - mTopCropped - mBottomCropped));
//        if (mVideoHeight % 2 != 0) {
//            mVideoHeight += 1; // Pixels must be even
//        }
//        int mVideoWidth = config.mWidth;
        // 创建MediaCodec编码器
        try {
            mVideoEncoder = new VideoEncoderCore(config);
